...
reg [31:0] mul_res;    ////temporary register to hold the entire multiplication
...
/////////////////sign bit
if(`oper_type == `mul)
  sign = SGPR[15];    //checking the MSB bit for sign
else                  //0 for positive and 1 for negative
  sign = GPR[`rdst][15];
...
if(`oper_type == `add)
   begin
      if(`imm_mode)
         ...
         carry    = temp_sum[16]; //temporary register to hold carry
         end
      else
         begin
         temp_sum = GPR[`rsrc1] + GPR[`rsrc2];
         carry    = temp_sum[16]; //checking the last bit of temporary register
         end   
         end
   else
    begin
        carry  = 1'b0;//carry being zero for other operations
    end
...
if(`oper_type == `mul)
  ...
  zero =  ~(|GPR[`rdst]);   ;//checking if all bits are 0 
...
initial begin
$readmemb("C:/Users/Pritish Panda/OneDrive/Desktop/processor_memory.mem",inst_mem);
end
